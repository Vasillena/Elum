...
//           {/* <GlowStrip /> */}
//           <Suspense fallback={null}>{/* <ELUMText /> */}</Suspense>
//         </BackgroundParallax>
...
//   // предишна позиция за изчисляване на скоростта
//   const lastMouse = useRef({ x: 0, y: 0 });

//   useFrame(() => {
//     if (!groupRef.current) return;

//     // изчисляваме "скорост" на мишката
//     const dx = mouse.current.x - lastMouse.current.x;
//     const dy = mouse.current.y - lastMouse.current.y;
//     lastMouse.current.x = mouse.current.x;
//     lastMouse.current.y = mouse.current.y;

//     // target rotation: добавяме малко движение според скоростта
//     target.current.y += dx * 1; // сила по Y
//     target.current.x += -dy * 0.5; // сила по X

//     // плавно приближаваме текущата rotation към target
//     current.current.x += (target.current.x - current.current.x) * 0.08;
//     current.current.y += (target.current.y - current.current.y) * 0.08;

//     // бавно връщаме target към центъра (0,0)
...
//               resolution={1024}
//               backside
//               background={new THREE.TextureLoader().load("/sep09.jpg")}
//             />
//           </mesh>
//         ) : null
//       )}
